...
cdef class FastTextModelWrapper:
    ...
    def dict_get_word_counts(self):
        return self.fm.dictGetWordCounts()

    def dict_get_label_counts(self):
        return self.fm.dictGetLabelCounts()
    ...
    @property
    def dropInput(self):
        return self.fm.dropInput

    @property
    def lrFreeze(self):
        return self.fm.lrFreeze
...
def train_wrapper(model_name, input_file, output, label_prefix, lr, dim, ws,
    ...
    # Load the model
    model = load_model(output + '.bin', label_prefix, encoding=encoding)
